...
const Idea = ({myIdea}: {myIdea: boolean}) => {
  ...
  const renderSpecialty = (data: string[]) => {
    ...
    return data.map((item, i) => {
      if (i < 2) {
        return (
          <li className='idea__container-specialty-item'>
            <span className='idea__container-specialty-item-status '></span>
            <span className='idea__container-specialty-item-text'>{item}</span>
          </li>
        ) 
      } else {
        return null  
  ...
  const allSpecialty = data.length > 1 ?
    <li className='idea__container-specialty-item all-specialty'>
      <span className='idea__container-specialty-item-status'></span>
      <span className='idea__container-specialty-item-text'>{`+${data.length - 2}`}</span>
    </li>
    :
    null
    ...
    <div className="idea">
      ...
      <div className="idea__container">
        <div className="idea__container-info">
            ...
            <p className='idea__container-info-description'>Шукаю бажаючих долучитись до розробки ідеї арт-сайту.</p>
        </div>
